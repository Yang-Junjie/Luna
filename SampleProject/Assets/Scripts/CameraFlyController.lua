local CameraFlyController = {}

CameraFlyController.Properties = {
    enabled = {
        type = "Bool",
        default = true,
...
local function normalized_axis(x, y, z)
...
function CameraFlyController:OnCreate()
    ...
    self.move_speed = self.move_speed or 6.0
    self.fast_multiplier = self.fast_multiplier or 4.0
    ...
    self._pitch = rotation.x
    self._yaw = rotation.y
    self._capturing = false
    self._ignore_next_mouse_delta = false

    if self.entity:has_camera() then
        self.entity:set_primary_camera(true)
    end
end

function CameraFlyController:_begin_capture()
...
function CameraFlyController:_end_capture()
...
function CameraFlyController:OnUpdate(dt)
    ...
    if Input.is_key_pressed(KeyCode.Space) then y = y + 1.0 end
    if Input.is_key_pressed(KeyCode.LeftControl) then y = y - 1.0 end

    x, y, z = normalized_axis(x, y, z)
    local speed = self.move_speed
    if Input.is_key_pressed(KeyCode.LeftShift) then
        speed = speed * self.fast_multiplier
    end

    self.entity:translate_local(Vec3(x * speed * dt, y * speed * dt, z * speed * dt))
end

function CameraFlyController:OnDestroy()
    self:_end_capture()
end

return CameraFlyController
