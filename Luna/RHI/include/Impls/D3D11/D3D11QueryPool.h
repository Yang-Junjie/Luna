...
namespace Cacao
{
    ...
    public:
        ...
        uint32_t GetCount() const override { return static_cast<uint32_t>(m_queries.size()); }
        ...
        bool GetResults(uint32_t firstQuery, uint32_t queryCount,
                        std::vector<uint64_t>& outResults, bool wait = true) override;
        ID3D11Query* GetQuery(uint32_t index) const { return m_queries[index].Get(); }
    };
}
