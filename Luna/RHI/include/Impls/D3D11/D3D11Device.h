...
namespace Cacao
{
    ...
    {
    public:
        D3D11Queue(Ref<D3D11Device> device) : m_device(std::move(device)) {}
    ...
    public:
        ...
        std::vector<uint32_t> GetAllQueueFamilyIndices() const override { return {0}; }
        ...
        ID3D11Device5* GetNativeDevice() const { return m_device.Get(); }
        ID3D11DeviceContext4* GetImmediateContext() const { return m_immediateContext.Get(); }
