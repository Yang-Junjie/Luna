...
namespace Cacao
{
    ...
    public:
        D3D12_CPU_DESCRIPTOR_HANDLE AllocateRTV() { return m_rtvPool.Allocate(); }
        D3D12_CPU_DESCRIPTOR_HANDLE AllocateDSV() { return m_dsvPool.Allocate(); }
        D3D12_CPU_DESCRIPTOR_HANDLE AllocateCbvSrvUav() { return m_cbvSrvUavPool.Allocate(); }

        ComPtr<ID3D12CommandAllocator> AcquireCommandAllocator();
        void RecycleCommandAllocator(ComPtr<ID3D12CommandAllocator> alloc);
        ID3D12PipelineLibrary1* GetPipelineLibrary() { return m_pipelineLibrary.Get(); }
