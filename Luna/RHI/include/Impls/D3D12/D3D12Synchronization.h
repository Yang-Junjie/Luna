...
namespace Cacao
{
    ...
    public:
        ...
        ID3D12Fence* GetFence(uint32_t frameIndex) const { return m_frameFences[frameIndex].Get(); }
        uint64_t GetFenceValue(uint32_t frameIndex) const { return m_fenceValues[frameIndex]; }
        void IncrementFenceValue(uint32_t frameIndex) { m_fenceValues[frameIndex]++; }
    };
}
