...
namespace Cacao
{
    ...
    public:
        ...
        void ComputePushConstants(const Ref<ComputePipeline>& pipeline, ShaderStage stageFlags,
                                  uint32_t offset, uint32_t size, const void* data) override;
        void CopyBuffer(const Ref<Buffer>& src, const Ref<Buffer>& dst,
                        uint64_t srcOffset, uint64_t dstOffset, uint64_t size) override;
        void CopyBufferToImage(const Ref<Buffer>& src, const Ref<Texture>& dst,
                               ResourceState dstState, std::span<const BufferImageCopy> regions) override;
        ...
        void ResolveTexture(const Ref<Texture>& src, const Ref<Texture>& dst,
        ...
        ::WGPUCommandBuffer GetFinishedCommandBuffer() const { return m_commandBuffer; }
    };
}
