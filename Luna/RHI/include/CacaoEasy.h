...
public:
    ...
    {
        auto buf = device->CreateBuffer(BufferBuilder()
                                            .SetSize(size)
    ...
    {
        return device->CreateBuffer(BufferBuilder()
                                        .SetSize(size)
    ...
    QuickPipeline CreateQuickPipeline(const std::string& shaderPath,
    ...
    void ClearAndBeginRendering(
        const Ref<CommandBufferEncoder>& cmd, float r = 0.1f, float g = 0.1f, float b = 0.1f, float a = 1.0f)
    {
        ...
        Viewport vp = {
            0, 0, static_cast<float>(swapchain->GetExtent().width), static_cast<float>(swapchain->GetExtent().height)};
        cmd->SetViewport(vp);
