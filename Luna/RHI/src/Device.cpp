...
namespace Cacao
{
    bool Device::ValidateGraphicsPipeline(const GraphicsPipelineCreateInfo& info) const
    {
        ...
        {
            throw std::runtime_error("[Cacao] Pipeline validation FAILED: " +
                std::to_string(info.ColorAttachmentFormats.size()) +
                " color attachments exceeds device limit of " +
        ...
        if (info.Multisample.RasterizationSamples > limits.maxMSAASamples)
        {
            throw std::runtime_error("[Cacao] Pipeline validation FAILED: MSAA " +
                std::to_string(info.Multisample.RasterizationSamples) +
                "x exceeds device limit of " + std::to_string(limits.maxMSAASamples) + "x");
        }
        ...
        {
            if (!info.Shaders[i])
                throw std::runtime_error("[Cacao] Pipeline validation FAILED: Shader[" +
