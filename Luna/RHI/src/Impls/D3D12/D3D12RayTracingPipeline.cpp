        ...
        : m_layout(info.Layout)
    {
        ...
        for (size_t i = 0; i < shaderCount; i++)
        {
            ...
            std::cerr << "  Shader[" << i << "]: stage=" << static_cast<int>(info.Shaders[i]->GetStage())
                      << " entry=" << info.Shaders[i]->GetEntryPoint()
                      << " size=" << blob.Data.size() << " bytes";
            if (blob.Data.size() >= 4) {
                std::cerr << " magic=0x" << std::hex
                    << (blob.Data[0] | (blob.Data[1]<<8) | (blob.Data[2]<<16) | (blob.Data[3]<<24))
                    << std::dec;
            }
            std::cerr << std::endl;
        }
        ...
        if (FAILED(hr))
        {
            ...
            char buf[256];
            snprintf(buf, sizeof(buf), "Failed to create D3D12 RT pipeline: HRESULT=0x%08lX, subobjects=%u", hr, stateObjDesc.NumSubobjects);
