...
namespace Cacao
{
    static D3D12_QUERY_HEAP_TYPE ToD3D12QueryHeapType(QueryType type)
    {
        ...
        default: return D3D12_QUERY_HEAP_TYPE_TIMESTAMP;
        }
    }
        ...
        : m_device(device), m_createInfo(info)
    {
        ...
        d3dDevice->GetAllocator()->CreateResource(
            &allocDesc, &bufDesc, D3D12_RESOURCE_STATE_COPY_DEST,
            nullptr, &m_readbackAllocation, IID_PPV_ARGS(&m_readbackBuffer));
    }
