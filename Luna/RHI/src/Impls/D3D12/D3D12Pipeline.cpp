...
namespace { std::atomic<uint64_t> g_psoCounter{0}; }

namespace Cacao
{
    D3D12GraphicsPipeline::D3D12GraphicsPipeline(const Ref<Device>& device,
                                                  const GraphicsPipelineCreateInfo& info)
        : m_device(device), m_layout(info.Layout), m_vertexBindings(info.VertexBindings)
    {
        ...
        if (!m_rootSignature)
        {
            ...
            d3dDevice->GetHandle()->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
        ...
        case CullMode::Back:  psoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_BACK; break;
        ...
        if (psoDesc.SampleDesc.Count == 0) psoDesc.SampleDesc.Count = 1;
    ...
    D3D12ComputePipeline::D3D12ComputePipeline(const Ref<Device>& device,
                                                const ComputePipelineCreateInfo& info)
        : m_device(device), m_layout(info.Layout)
    {
        auto d3dDevice = std::dynamic_pointer_cast<D3D12Device>(device);
