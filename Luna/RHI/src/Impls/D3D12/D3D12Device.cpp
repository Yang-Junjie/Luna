...
namespace Cacao
{
        ...
        : m_parentAdapter(adapter)
    {
        ...
        {
            char buf[128];
            snprintf(buf, sizeof(buf), "Device already removed after creation: 0x%08X", static_cast<unsigned>(removedReason));
            throw std::runtime_error(buf);
        }
        ...
        {
            char buf[128];
            snprintf(buf, sizeof(buf), "Device removed after CreateDescriptorHeaps: 0x%08X", static_cast<unsigned>(removedReason));
            throw std::runtime_error(buf);
        }
    }
        ...
        if (file.is_open())
        {
            ...
            if (size > 0)
            {
                ...
                HRESULT hr = m_device->CreatePipelineLibrary(
                    m_psoLibraryBlob.data(), m_psoLibraryBlob.size(),
    ...
    {
        if (!m_pipelineLibrary || !pso) return;
    ...
    void D3D12Device::CreateDescriptorHeaps()
    {
        ...
        createHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 65536, true, m_cbvSrvUavHeap);
        createHeap(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, 2048, true, m_samplerHeap);

        auto initPool = [](DescriptorPoolAllocator& pool, ComPtr<ID3D12DescriptorHeap>& heap,
                           uint32_t descSize, uint32_t capacity) {
            pool.heap = heap;
        ...
        initPool(m_cbvSrvUavPool, m_cbvSrvUavHeap, m_cbvSrvUavDescriptorSize, 65536);
    }
    ...
    std::vector<uint32_t> D3D12Device::GetAllQueueFamilyIndices() const { return {0}; }
    Ref<Adapter> D3D12Device::GetParentAdapter() const { return m_parentAdapter; }
    ...
    void D3D12Device::ReturnCommandBuffer(const Ref<CommandBufferEncoder>& encoder) { /* no-op for D3D12 */ }
    void D3D12Device::FreeCommandBuffer(const Ref<CommandBufferEncoder>& encoder) { /* no-op for D3D12 */ }
    void D3D12Device::ResetCommandBuffer(const Ref<CommandBufferEncoder>& encoder) { encoder->Reset(); }
    ...
    Ref<ShaderBindingTable> D3D12Device::CreateShaderBindingTable(
