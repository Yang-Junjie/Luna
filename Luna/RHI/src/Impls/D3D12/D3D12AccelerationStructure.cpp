...
namespace Cacao
{
        ...
        : m_type(info.Type), m_device(device)
    {
        ...
        m_inputs.Flags = info.PreferFastTrace
        ...
        if (info.Type == AccelerationStructureType::BottomLevel) {
            for (auto& g : info.Geometries) {
                ...
                gd.Flags = g.Opaque ? D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE
                ...
                gd.Triangles.VertexFormat = (g.VertexFormat != Format::UNDEFINED)
                    ? ToDXGIFormat(g.VertexFormat) : DXGI_FORMAT_R32G32B32_FLOAT;
                ...
                if (g.IndexBuffer) {
                    ...
                    gd.Triangles.IndexFormat = (g.IndexFormat == IndexType::UInt16)
        ...
        fprintf(stderr, "[AS] type=%d numDescs=%u resultSize=%llu scratchSize=%llu\n",
            static_cast<int>(info.Type), m_inputs.NumDescs, m_resultSize, m_scratchSize);
        if (info.Type == AccelerationStructureType::BottomLevel && !m_geomDescs.empty()) {
            auto& g = m_geomDescs[0];
            fprintf(stderr, "[AS] geom: type=%d vertCount=%u vertFormat=%d vertStride=%llu indexCount=%u addr=0x%llx\n",
                g.Type, g.Triangles.VertexCount, g.Triangles.VertexFormat,
                g.Triangles.VertexBuffer.StrideInBytes, g.Triangles.IndexCount,
        ...
        dev5->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
            D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr, IID_PPV_ARGS(&m_resultBuffer));

        rd.Width = m_scratchSize;
        dev5->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
            D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_scratchBuffer));
        ...
        if (info.Type == AccelerationStructureType::TopLevel && !info.Instances.empty()) {
            ...
            dev5->CreateCommittedResource(&uhp, D3D12_HEAP_FLAG_NONE, &rd,
