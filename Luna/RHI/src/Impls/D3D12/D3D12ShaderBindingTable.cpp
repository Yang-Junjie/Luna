...
namespace Cacao
{
    ...
    D3D12ShaderBindingTable::D3D12ShaderBindingTable(
        const Ref<Device>& device, ID3D12StateObject* rtPSO,
        ...
        uint32_t hitGroupCount, uint32_t callableCount)
    {
        ...
        auto copyIdentifier = [&](uint64_t offset, const wchar_t* name) {
            ...
            if (id) memcpy(data + offset, id, D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
        };

        copyIdentifier(m_rayGenOffset, L"RayGen");
        if (missCount > 0) copyIdentifier(m_missOffset, L"Miss");
        if (hitGroupCount > 0) copyIdentifier(m_hitGroupOffset, L"HitGroup");
    ...
    D3D12_DISPATCH_RAYS_DESC D3D12ShaderBindingTable::GetDispatchRaysDesc(
