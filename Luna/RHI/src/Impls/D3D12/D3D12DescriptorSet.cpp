...
namespace Cacao
{
    D3D12DescriptorSet::D3D12DescriptorSet(const Ref<Device>& device,
        ...
        : m_device(device)
        , m_cbvSrvUavGpu(cbvGpu), m_cbvSrvUavCpu(cbvCpu)
        , m_cbvSrvUavCount(cbvCount), m_cbvSrvUavDescSize(cbvDescSize)
        , m_samplerGpu(sampGpu), m_samplerCpu(sampCpu)
        , m_samplerCount(sampCount), m_samplerDescSize(sampDescSize)
        , m_bindingMap(std::move(bindingMap))
        , m_cbvSrvUavHeap(cbvHeap), m_samplerHeap(sampHeap)
    {
    }
    ...
    void D3D12DescriptorSet::WriteTexture(const TextureWriteInfo& info)
    {
        ...
        d3dDevice->GetHandle()->CopyDescriptorsSimple(1, dest, srcHandle,
    ...
    void D3D12DescriptorSet::WriteSampler(const SamplerWriteInfo& info)
    {
        ...
        if (!d3dDevice || !d3dSampler) return;
        ...
        d3dDevice->GetHandle()->CopyDescriptorsSimple(1, dest, d3dSampler->GetCPUHandle(),
