...
namespace Cacao
{
    ...
    Result D3D12Swapchain::Present(const Ref<Queue>& queue, const Ref<Synchronization>& sync,
                                    uint32_t frameIndex)
    {
        ...
        if (SUCCEEDED(hr)) return Result::Success;
            ...
            if (d3dDev)
            {
                ...
                if (SUCCEEDED(d3dDev->GetHandle()->QueryInterface(IID_PPV_ARGS(&iq))))
                {
                    ...
                    for (UINT64 i = 0; i < n && i < 10; ++i)
                    {
                        ...
                        if (msg) { iq->GetMessage(i, msg, &len); fprintf(stderr, "  D3D12 MSG[%llu]: %s\n", i, msg->pDescription); free(msg); }
                    }
                }
            }
