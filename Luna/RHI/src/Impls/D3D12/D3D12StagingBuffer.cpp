...
namespace Cacao
{
    D3D12StagingBufferPool::D3D12StagingBufferPool(const Ref<Device>& device, uint64_t blockSize,
                                                   uint32_t maxFramesInFlight)
        : m_device(device), m_blockSize(blockSize)
    {
        ...
        d3dDevice->GetAllocator()->CreateResource(
            &allocDesc, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
            nullptr, &m_allocation, IID_PPV_ARGS(&m_uploadBuffer));
