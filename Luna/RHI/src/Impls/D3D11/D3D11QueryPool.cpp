...
namespace Cacao
{
    static D3D11_QUERY ToD3D11QueryType(QueryType type)
    {
        ...
        default: return D3D11_QUERY_TIMESTAMP;
        }
    }
    ...
    bool D3D11QueryPool::GetResults(uint32_t firstQuery, uint32_t queryCount,
                                     std::vector<uint64_t>& outResults, bool wait)
    {
        ...
        {
            uint64_t result = 0;
            HRESULT hr = ctx->GetData(m_queries[firstQuery + i].Get(), &result, sizeof(uint64_t), wait ? 0 : D3D11_ASYNC_GETDATA_DONOTFLUSH);
