...
namespace Cacao
{
    D3D11DescriptorSet::D3D11DescriptorSet(Ref<D3D11Device> device, Ref<D3D11DescriptorSetLayout> layout)
        : m_device(std::move(device)), m_layout(std::move(layout))
    {
    ...
    void D3D11DescriptorSet::WriteTextures(const TextureWriteInfos& infos)
    {
        for (size_t i = 0; i < infos.TextureViews.size(); ++i)
        {
            ...
            single.Type = infos.Type;
            single.ArrayElement = infos.ArrayElement + static_cast<uint32_t>(i);
            if (i < infos.Samplers.size()) single.Sampler = infos.Samplers[i];
            WriteTexture(single);
        }
    }
            ...
            case DescriptorType::UniformBufferDynamic:
                {
                    ...
                    if (!cb) break;
                    if (stages & ShaderStage::Vertex) ctx->VSSetConstantBuffers(res.slot, 1, &cb);
                    if (stages & ShaderStage::Fragment) ctx->PSSetConstantBuffers(res.slot, 1, &cb);
                    if (stages & ShaderStage::Compute) ctx->CSSetConstantBuffers(res.slot, 1, &cb);
                }
                break;
            ...
            case DescriptorType::CombinedImageSampler:
                {
                    ...
                    if (!srv) break;
                    if (stages & ShaderStage::Vertex) ctx->VSSetShaderResources(res.slot, 1, &srv);
                    if (stages & ShaderStage::Fragment) ctx->PSSetShaderResources(res.slot, 1, &srv);
                    if (stages & ShaderStage::Compute) ctx->CSSetShaderResources(res.slot, 1, &srv);

                    ID3D11SamplerState* sampler = res.sampler.Get();
                    if (sampler)
                    {
                        if (stages & ShaderStage::Vertex) ctx->VSSetSamplers(res.slot, 1, &sampler);
                        if (stages & ShaderStage::Fragment) ctx->PSSetSamplers(res.slot, 1, &sampler);
                        if (stages & ShaderStage::Compute) ctx->CSSetSamplers(res.slot, 1, &sampler);
                    }
                }
                break;
            ...
            case DescriptorType::StorageImage:
                {
                    ...
                    if (!uav) break;
                    if (stages & ShaderStage::Compute) ctx->CSSetUnorderedAccessViews(res.slot, 1, &uav, nullptr);
                }
                break;
    ...
    void D3D11DescriptorPool::Reset() { /* DX11: no-op, resources are individually managed */ }
