...
namespace Cacao
{
    D3D11Swapchain::D3D11Swapchain(Ref<D3D11Device> device, const SwapchainCreateInfo& createInfo)
        : m_device(std::move(device)), m_extent(createInfo.Extent),
          m_format(createInfo.Format), m_presentMode(createInfo.PresentMode),
          m_imageCount(createInfo.MinImageCount)
    {
        ...
        if (createInfo.CompatibleSurface)
        {
            ...
            if (d3dSurface) hwnd = d3dSurface->GetHWND();
        }
        ...
        if (m_swapchain) CreateBackBuffers();
    }
    ...
    {
        if (!m_swapchain) return Result::Error;
