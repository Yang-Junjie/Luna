...
namespace Cacao
{
    D3D11Buffer::D3D11Buffer(Ref<D3D11Device> device, const BufferCreateInfo& createInfo)
        : m_device(std::move(device)), m_size(createInfo.Size),
          m_usage(createInfo.Usage), m_memoryUsage(createInfo.MemoryUsage)
    {
        ...
        if (m_usage & BufferUsageFlags::VertexBuffer) desc.BindFlags |= D3D11_BIND_VERTEX_BUFFER;
        if (m_usage & BufferUsageFlags::IndexBuffer) desc.BindFlags |= D3D11_BIND_INDEX_BUFFER;
        if (m_usage & BufferUsageFlags::UniformBuffer) desc.BindFlags |= D3D11_BIND_CONSTANT_BUFFER;
        if (m_usage & BufferUsageFlags::StorageBuffer) desc.BindFlags |= D3D11_BIND_SHADER_RESOURCE;
    ...
    {
        if (m_isMapped) return m_needsStagingForMap ? m_cpuStagingData.data() : m_mappedData.pData;
        ...
        D3D11_MAP mapType = (desc.Usage == D3D11_USAGE_DYNAMIC)
            ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_READ_WRITE;

        HRESULT hr = m_device->GetImmediateContext()->Map(
            m_buffer.Get(), 0, mapType, 0, &m_mappedData);
    ...
    void D3D11Buffer::Unmap()
    {
        ...
        if (m_needsStagingForMap)
        {
            m_device->GetImmediateContext()->UpdateSubresource(
                m_buffer.Get(), 0, nullptr, m_cpuStagingData.data(), 0, 0);
        }
        else
        {
    ...
    {
        if (m_structuredSRV && m_structuredStride == stride) return;
        ...
        fprintf(stderr, "  CreateSRV hr=0x%08X srv=%p structured=%p\n", (unsigned)hr, m_structuredSRV.Get(), m_structuredBuffer.Get());
    }
}
