...
namespace Cacao
{
    ...
    {
        return CreateRef<D3D11Swapchain>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), createInfo);
    }

    Ref<Adapter> D3D11Device::GetParentAdapter() const { return m_adapter; }

    Ref<CommandBufferEncoder> D3D11Device::CreateCommandBufferEncoder(CommandBufferType type)
    {
        return CreateRef<D3D11CommandBufferEncoder>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), type);
    }

    Ref<Texture> D3D11Device::CreateTexture(const TextureCreateInfo& createInfo)
    {
        return CreateRef<D3D11Texture>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), createInfo);
    }

    Ref<Buffer> D3D11Device::CreateBuffer(const BufferCreateInfo& createInfo)
    {
        return CreateRef<D3D11Buffer>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), createInfo);
    }

    Ref<Sampler> D3D11Device::CreateSampler(const SamplerCreateInfo& createInfo)
    {
        return CreateRef<D3D11Sampler>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), createInfo);
    }
    ...
    {
        return CreateRef<D3D11DescriptorPool>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), info);
    }
    ...
    Ref<GraphicsPipeline> D3D11Device::CreateGraphicsPipeline(const GraphicsPipelineCreateInfo& info)
    {
        ...
        return CreateRef<D3D11GraphicsPipeline>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), info);
    }

    Ref<ComputePipeline> D3D11Device::CreateComputePipeline(const ComputePipelineCreateInfo& info)
    {
        return CreateRef<D3D11ComputePipeline>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), info);
    }

    Ref<Synchronization> D3D11Device::CreateSynchronization(uint32_t maxFramesInFlight)
    {
        return CreateRef<D3D11Synchronization>(
            std::static_pointer_cast<D3D11Device>(shared_from_this()), maxFramesInFlight);
    }
}
