...
namespace Cacao
{
    static D3D11_CULL_MODE ToD3D11CullMode(CullMode mode)
    {
        ...
        case CullMode::FrontAndBack: return D3D11_CULL_NONE;
    ...
    static D3D11_STENCIL_OP ToD3D11StencilOp(StencilOp op)
    {
        ...
        case StencilOp::IncrementAndClamp: return D3D11_STENCIL_OP_INCR_SAT;
        case StencilOp::DecrementAndClamp: return D3D11_STENCIL_OP_DECR_SAT;
        ...
        : m_device(std::move(device))
    {
        ...
        for (auto& shader : info.Shaders)
        {
            ...
            if (mod->GetStage() == ShaderStage::Vertex) vsModule = mod;
            else if (mod->GetStage() == ShaderStage::Fragment) psModule = mod;
            else if (mod->GetStage() == ShaderStage::Geometry) gsModule = mod;
        }
        ...
        if (vsModule)
        {
            ...
            if (!elements.empty())
                dev->CreateInputLayout(elements.data(), static_cast<UINT>(elements.size()),
                    bc.data(), bc.size(), &m_inputLayout);
        }
...

        {
            ...
            dsd.DepthWriteMask = info.DepthStencil.DepthWriteEnable
            ...
            auto mapStencilFace = [](const StencilOpState& s) -> D3D11_DEPTH_STENCILOP_DESC {
                return {ToD3D11StencilOp(s.FailOp), ToD3D11StencilOp(s.DepthFailOp),
                        ToD3D11StencilOp(s.PassOp), D3D11_ToCompareFunc(s.CompareOp)};
            };
...

        {
            ...
            for (size_t i = 0; i < info.ColorBlend.Attachments.size() && i < 8; ++i)
            {
                ...
                if (src.ColorWriteMask & ColorComponentFlags::R) dst.RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_RED;
                if (src.ColorWriteMask & ColorComponentFlags::G) dst.RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_GREEN;
                if (src.ColorWriteMask & ColorComponentFlags::B) dst.RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_BLUE;
                if (src.ColorWriteMask & ColorComponentFlags::A) dst.RenderTargetWriteMask |= D3D11_COLOR_WRITE_ENABLE_ALPHA;
            }
        ...
        case PrimitiveTopology::PatchList:
            m_topology = D3D11_PRIMITIVE_TOPOLOGY(
                D3D11_PRIMITIVE_TOPOLOGY_1_CONTROL_POINT_PATCHLIST + info.InputAssembly.PatchControlPoints - 1);
            break;
        default: m_topology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST; break;
        }
    }
