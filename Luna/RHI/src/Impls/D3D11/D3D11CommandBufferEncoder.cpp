...
namespace Cacao
{
        ...
        : m_device(std::move(device)), m_type(type)
    {
        ...
        {
            m_device->GetNativeDevice()->CreateDeferredContext(0,
    ...
    void D3D11CommandBufferEncoder::Reset() { /* DX11: no-op for immediate context */ }
    void D3D11CommandBufferEncoder::Begin(const CommandBufferBeginInfo& info) { /* no-op */ }
    void D3D11CommandBufferEncoder::End() { /* no-op for immediate context */ }
    ...
    void D3D11CommandBufferEncoder::BeginRendering(const RenderingInfo& info)
    {
        ...
        for (auto& att : info.ColorAttachments)
        {
            ...
            {
                fprintf(stderr, "[Cacao WARNING] DX11: Texture used in BeginRendering without TransitionImage(). "
        ...
        if (info.DepthAttachment)
        {
            ...
            if (info.DepthAttachment->LoadOp == AttachmentLoadOp::Clear)
                m_context->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL,
    ...
    void D3D11CommandBufferEncoder::BindGraphicsPipeline(const Ref<GraphicsPipeline>& pipeline)
    {
        ...
        m_context->OMSetBlendState(p->GetBlendState(), blendFactor, 0xFFFFFFFF);
    ...
    {
        D3D11_VIEWPORT vp{viewport.X, viewport.Y, viewport.Width, viewport.Height,
    ...
    {
        D3D11_RECT r{scissor.OffsetX, scissor.OffsetY,
    ...
    void D3D11CommandBufferEncoder::BindVertexBuffer(uint32_t binding, const Ref<Buffer>& buffer, uint64_t offset)
    {
        ...
        if (buffer && !(buffer->GetUsage() & BufferUsageFlags::VertexBuffer))
            fprintf(stderr, "[Cacao WARNING] DX11: BindVertexBuffer without VertexBuffer usage flag. "
    ...
    void D3D11CommandBufferEncoder::BindIndexBuffer(const Ref<Buffer>& buffer, uint64_t offset, IndexType indexType)
    {
        ...
        if (buffer && !(buffer->GetUsage() & BufferUsageFlags::IndexBuffer))
            fprintf(stderr, "[Cacao WARNING] DX11: BindIndexBuffer without IndexBuffer usage flag. "
                    "Vulkan will reject this.\n");
#endif
        auto* d3dBuf = static_cast<D3D11Buffer*>(buffer.get());
    ...
    void D3D11CommandBufferEncoder::BindDescriptorSets(const Ref<GraphicsPipeline>& pipeline,
        uint32_t firstSet, std::span<const Ref<DescriptorSet>> descriptorSets)
    {
    ...
    void D3D11CommandBufferEncoder::PushConstants(const Ref<GraphicsPipeline>& pipeline,
        ShaderStage stageFlags, uint32_t offset, uint32_t size, const void* data)
    {
        ...
        if (stageFlags & ShaderStage::Vertex) m_context->VSSetConstantBuffers(slot, 1, bufs);
        if (stageFlags & ShaderStage::Fragment) m_context->PSSetConstantBuffers(slot, 1, bufs);
        if (stageFlags & ShaderStage::Geometry) m_context->GSSetConstantBuffers(slot, 1, bufs);
    }

    void D3D11CommandBufferEncoder::Draw(uint32_t vertexCount, uint32_t instanceCount,
    ...
    void D3D11CommandBufferEncoder::DrawIndexed(uint32_t indexCount, uint32_t instanceCount,
    ...
    void D3D11CommandBufferEncoder::ComputePushConstants(const Ref<ComputePipeline>& pipeline,
        ShaderStage stageFlags, uint32_t offset, uint32_t size, const void* data)
    {
        uint32_t alignedSize = (size + 15) & ~15;
        D3D11_BUFFER_DESC desc{};
    ...
    void D3D11CommandBufferEncoder::PipelineBarrier(SyncScope srcStage, SyncScope dstStage,
    ...
    void D3D11CommandBufferEncoder::TransitionImage(const Ref<Texture>& texture,
        ImageTransition transition, const ImageSubresourceRange& range)
    {
#ifndef NDEBUG
        if (texture)
    ...
    void D3D11CommandBufferEncoder::TransitionBuffer(const Ref<Buffer>& buffer,
        BufferTransition transition, uint64_t offset, uint64_t size)
    {
        // DX11: no-op
    }
        ...
        std::span<const BufferImageCopy> regions)
    {
        ...
        for (auto& region : regions)
        {
            ...
            uint32_t rowPitch = region.BufferRowLength > 0
                ? region.BufferRowLength
                : region.ImageExtentWidth * 4;
            ...
            m_context->UpdateSubresource(
                dstTex->GetNativeTexture(), subresource,
                &dstBox, srcData, rowPitch, 0);
        }
    ...
    void D3D11CommandBufferEncoder::CopyTexture2D(const Ref<Texture>& src, const Ref<Texture>& dst)
    {
        ...
        else
        {
            D3D11_BOX box = {0, 0, 0,
                ...
                std::min(src->GetHeight(), dst->GetHeight()), 1};
            m_context->CopySubresourceRegion(dstTex->GetNativeTexture(), 0, 0, 0, 0,
                srcTex->GetNativeTexture(), 0, &box);
        }
    }

    void D3D11CommandBufferEncoder::CopyBuffer(const Ref<Buffer>& srcBuffer,
        const Ref<Buffer>& dstBuffer, uint64_t srcOffset, uint64_t dstOffset, uint64_t size)
    {
        auto* src = static_cast<D3D11Buffer*>(srcBuffer.get());
        auto* dst = static_cast<D3D11Buffer*>(dstBuffer.get());
        D3D11_BOX box{static_cast<UINT>(srcOffset), 0, 0,
                      static_cast<UINT>(srcOffset + size), 1, 1};
        m_context->CopySubresourceRegion(dst->GetNativeBuffer(), 0,
            static_cast<UINT>(dstOffset), 0, 0, src->GetNativeBuffer(), 0, &box);
    }

    void D3D11CommandBufferEncoder::ResolveTexture(const Ref<Texture>& srcTexture,
        const Ref<Texture>& dstTexture, const ImageSubresourceLayers& srcSub,
        const ImageSubresourceLayers& dstSub)
    {
        ...
        m_context->ResolveSubresource(dst->GetNativeTexture(), dstSub.MipLevel,
                                       src->GetNativeTexture(), srcSub.MipLevel, format);
    }

    void D3D11CommandBufferEncoder::DrawIndirect(const Ref<Buffer>& argBuffer, uint64_t offset,
                                                  uint32_t drawCount, uint32_t stride)
    {
        auto* buf = static_cast<D3D11Buffer*>(argBuffer.get());
        for (uint32_t i = 0; i < drawCount; ++i)
        {
            m_context->DrawInstancedIndirect(buf->GetNativeBuffer(),
                static_cast<UINT>(offset + i * stride));
        }
    }

    void D3D11CommandBufferEncoder::DrawIndexedIndirect(const Ref<Buffer>& argBuffer, uint64_t offset,
                                                         uint32_t drawCount, uint32_t stride)
    {
        auto* buf = static_cast<D3D11Buffer*>(argBuffer.get());
        for (uint32_t i = 0; i < drawCount; ++i)
        {
            m_context->DrawIndexedInstancedIndirect(buf->GetNativeBuffer(),
                static_cast<UINT>(offset + i * stride));
        }
    }
