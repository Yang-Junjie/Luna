...
namespace Cacao
{
    Ref<Texture> D3D11TextureView::GetTexture() const { return m_texture; }

    D3D11Texture::D3D11Texture(Ref<D3D11Device> device, const TextureCreateInfo& createInfo)
        : m_device(std::move(device)), m_width(createInfo.Width), m_height(createInfo.Height),
          m_depth(createInfo.Depth), m_mipLevels(createInfo.MipLevels),
          m_arrayLayers(createInfo.ArrayLayers), m_format(createInfo.Format),
          m_type(createInfo.Type), m_sampleCount(createInfo.SampleCount),
          m_usage(createInfo.Usage), m_currentState(createInfo.InitialState)
    {
        ...
        if (m_usage & TextureUsageFlags::Sampled) desc.BindFlags |= D3D11_BIND_SHADER_RESOURCE;
        if (m_usage & TextureUsageFlags::ColorAttachment) desc.BindFlags |= D3D11_BIND_RENDER_TARGET;
        if (m_usage & TextureUsageFlags::DepthStencilAttachment) desc.BindFlags |= D3D11_BIND_DEPTH_STENCIL;
        if (m_usage & TextureUsageFlags::Storage) desc.BindFlags |= D3D11_BIND_UNORDERED_ACCESS;
        ...
        if (createInfo.InitialData)
        {
            ...
            switch (m_format)
            {
            case Format::R8_UNORM: bpp = 1; break;
            case Format::RG8_UNORM: bpp = 2; break;
            case Format::R16_FLOAT: bpp = 2; break;
            case Format::RG16_FLOAT: bpp = 4; break;
            case Format::RGBA16_FLOAT: bpp = 8; break;
            case Format::R32_FLOAT: bpp = 4; break;
            case Format::RG32_FLOAT: bpp = 8; break;
            case Format::RGB32_FLOAT: bpp = 12; break;
        ...
        : m_device(std::move(device)), m_texture(std::move(existingTexture)), m_format(format)
    {
        ...
        if (desc.BindFlags & D3D11_BIND_RENDER_TARGET) m_usage = m_usage | TextureUsageFlags::ColorAttachment;
        if (desc.BindFlags & D3D11_BIND_SHADER_RESOURCE) m_usage = m_usage | TextureUsageFlags::Sampled;
        if (desc.BindFlags & D3D11_BIND_DEPTH_STENCIL) m_usage = m_usage | TextureUsageFlags::DepthStencilAttachment;
    ...
    Ref<CacaoTextureView> D3D11Texture::GetDefaultView() { return m_defaultView; }

    void D3D11Texture::CreateDefaultViewIfNeeded()
    {
        if (m_defaultView) return;
        m_defaultView = CreateRef<D3D11TextureView>(
            std::static_pointer_cast<D3D11Texture>(shared_from_this()));
    }
}
