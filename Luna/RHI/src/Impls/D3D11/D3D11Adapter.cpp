...
AdapterProperties D3D11Adapter::GetProperties() const
{
    ...
    return AdapterProperties{.deviceID = m_desc.DeviceId,
...
{
    if (m_desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) {
        return AdapterType::Software;
    }
    if (m_desc.DedicatedVideoMemory > 0) {
...
{
    return CreateRef<D3D11Device>(std::static_pointer_cast<D3D11Adapter>(shared_from_this()));
}
...
DeviceLimits D3D11Adapter::QueryLimits() const
{
    ...
    limits.maxStorageBufferSize = 128 * 1'024 * 1'024;
