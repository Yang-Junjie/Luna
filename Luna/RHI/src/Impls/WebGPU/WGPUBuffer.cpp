...
namespace Cacao
{
    WGPUBufferImpl::WGPUBufferImpl(::WGPUDevice device, const BufferCreateInfo& info)
        : m_wgpuDevice(device), m_createInfo(info)
    {
        WGPUBufferDescriptor desc = {};
        desc.size = info.Size;
        desc.usage = ToWGPUBufferUsage(info.Usage, info.MemoryUsage);
        desc.mappedAtCreation = (info.MemoryUsage == BufferMemoryUsage::CpuToGpu ||
                                 info.MemoryUsage == BufferMemoryUsage::CpuOnly);
        if (!info.Name.empty())
    ...
    uint64_t WGPUBufferImpl::GetSize() const { return m_createInfo.Size; }
    BufferUsageFlags WGPUBufferImpl::GetUsage() const { return m_createInfo.Usage; }
    BufferMemoryUsage WGPUBufferImpl::GetMemoryUsage() const { return m_createInfo.MemoryUsage; }
