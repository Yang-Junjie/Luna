...
namespace Cacao
{
    WGPUTextureFormat ToWGPUFormat(Format format)
    {
        ...
        case Format::RG11B10_FLOAT:  return WGPUTextureFormat_RG11B10Ufloat;
        ...
        case Format::S8_UINT:        return WGPUTextureFormat_Stencil8;
    ...
    Format FromWGPUFormat(WGPUTextureFormat format)
    {
        ...
        case WGPUTextureFormat_Depth32Float:   return Format::D32F;
    ...
    WGPUAddressMode ToWGPUAddressMode(SamplerAddressMode mode)
    {
        ...
        case SamplerAddressMode::MirroredRepeat:  return WGPUAddressMode_MirrorRepeat;
    ...
    WGPUColorWriteMaskFlags ToWGPUColorWriteMask(ColorComponentFlags flags)
    {
        ...
        if (flags & ColorComponentFlags::R) mask |= WGPUColorWriteMask_Red;
        if (flags & ColorComponentFlags::G) mask |= WGPUColorWriteMask_Green;
        if (flags & ColorComponentFlags::B) mask |= WGPUColorWriteMask_Blue;
        if (flags & ColorComponentFlags::A) mask |= WGPUColorWriteMask_Alpha;
        return mask;
    }
    ...
    WGPUBufferUsageFlags ToWGPUBufferUsage(BufferUsageFlags usage)
    {
        ...
        if (usage & BufferUsageFlags::TransferSrc)    flags |= WGPUBufferUsage_CopySrc;
        if (usage & BufferUsageFlags::TransferDst)    flags |= WGPUBufferUsage_CopyDst;
        if (usage & BufferUsageFlags::UniformBuffer)  flags |= WGPUBufferUsage_Uniform;
        if (usage & BufferUsageFlags::StorageBuffer)  flags |= WGPUBufferUsage_Storage;
        if (usage & BufferUsageFlags::IndexBuffer)    flags |= WGPUBufferUsage_Index;
        if (usage & BufferUsageFlags::VertexBuffer)   flags |= WGPUBufferUsage_Vertex;
        if (usage & BufferUsageFlags::IndirectBuffer) flags |= WGPUBufferUsage_Indirect;
        return flags;
    }
    ...
    WGPUTextureUsageFlags ToWGPUTextureUsage(TextureUsageFlags usage)
    {
        ...
        if (usage & TextureUsageFlags::TransferSrc)            flags |= WGPUTextureUsage_CopySrc;
        if (usage & TextureUsageFlags::TransferDst)            flags |= WGPUTextureUsage_CopyDst;
        if (usage & TextureUsageFlags::Sampled)                flags |= WGPUTextureUsage_TextureBinding;
        if (usage & TextureUsageFlags::Storage)                flags |= WGPUTextureUsage_StorageBinding;
        if (usage & TextureUsageFlags::ColorAttachment)        flags |= WGPUTextureUsage_RenderAttachment;
        if (usage & TextureUsageFlags::DepthStencilAttachment) flags |= WGPUTextureUsage_RenderAttachment;
        return flags;
    }
}
