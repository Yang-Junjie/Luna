...
void WGPUCommandBufferEncoder::BeginRendering(const RenderingInfo& info)
{
    ...
    for (auto& att : info.ColorAttachments) {
        ...
        ca.clearValue = {
            att.ClearValue.Color[0], att.ClearValue.Color[1], att.ClearValue.Color[2], att.ClearValue.Color[3]};
    ...
    if (info.DepthAttachment) {
        ...
        depthAtt.depthLoadOp =
            (info.DepthAttachment->LoadOp == AttachmentLoadOp::Clear) ? WGPULoadOp_Clear : WGPULoadOp_Load;
        depthAtt.depthStoreOp =
            (info.DepthAttachment->StoreOp == AttachmentStoreOp::Store) ? WGPUStoreOp_Store : WGPUStoreOp_Discard;
...
void WGPUCommandBufferEncoder::SetViewport(const Viewport& viewport)
{
    ...
    wgpuRenderPassEncoderSetViewport(
        m_renderPass, viewport.X, viewport.Y, viewport.Width, viewport.Height, viewport.MinDepth, viewport.MaxDepth);
}
...
void WGPUCommandBufferEncoder::SetScissor(const Rect2D& scissor)
{
    ...
    wgpuRenderPassEncoderSetScissorRect(m_renderPass,
                                        static_cast<uint32_t>(scissor.OffsetX),
                                        static_cast<uint32_t>(scissor.OffsetY),
                                        scissor.Width,
                                        scissor.Height);
}
...
void WGPUCommandBufferEncoder::BindVertexBuffer(uint32_t binding, const Ref<Buffer>& buffer, uint64_t offset)
{
    ...
    wgpuRenderPassEncoderSetVertexBuffer(
        m_renderPass, binding, buf->GetNativeBuffer(), offset, buf->GetSize() - offset);
}
...
void WGPUCommandBufferEncoder::BindIndexBuffer(const Ref<Buffer>& buffer, uint64_t offset, IndexType indexType)
{
    ...
    wgpuRenderPassEncoderSetIndexBuffer(m_renderPass, buf->GetNativeBuffer(), fmt, offset, buf->GetSize() - offset);
}

void WGPUCommandBufferEncoder::BindDescriptorSets(const Ref<GraphicsPipeline>& pipeline,
                                                  uint32_t firstSet,
...
void WGPUCommandBufferEncoder::PushConstants(const Ref<GraphicsPipeline>&, ShaderStage, uint32_t, uint32_t, const void*)
{
    // WebGPU has no push constants; emulate via uniform buffer
}

void WGPUCommandBufferEncoder::ComputePushConstants(
    const Ref<ComputePipeline>&, ShaderStage, uint32_t, uint32_t, const void*)
{
    // WebGPU has no push constants
}

void WGPUCommandBufferEncoder::Draw(uint32_t vertexCount,
...
void WGPUCommandBufferEncoder::DrawIndexed(
...
void WGPUCommandBufferEncoder::BindComputeDescriptorSets(const Ref<ComputePipeline>& pipeline,
                                                         uint32_t firstSet,
                                                         std::span<const Ref<DescriptorSet>> descriptorSets)
{
    if (!m_computePass) {
...
void WGPUCommandBufferEncoder::CopyBuffer(
    const Ref<Buffer>& src, const Ref<Buffer>& dst, uint64_t srcOffset, uint64_t dstOffset, uint64_t size)
{
    if (!m_encoder) {
        return;
    }
    ...
    wgpuCommandEncoderCopyBufferToBuffer(
        m_encoder, srcBuf->GetNativeBuffer(), srcOffset, dstBuf->GetNativeBuffer(), dstOffset, size);
}

void WGPUCommandBufferEncoder::CopyBufferToImage(const Ref<Buffer>& src,
                                                 ...
                                                 std::span<const BufferImageCopy> regions)
{
    ...
    for (auto& region : regions) {
        ...
        srcCopy.layout.bytesPerRow =
            region.BufferRowLength > 0 ? region.BufferRowLength * 4 : region.ImageExtentWidth * 4;
        srcCopy.layout.rowsPerImage =
            region.BufferImageHeight > 0 ? region.BufferImageHeight : region.ImageExtentHeight;

        WGPUImageCopyTexture dstCopy = {};
        dstCopy.texture = dstTex->GetNativeTexture();
        dstCopy.mipLevel = region.ImageSubresource.MipLevel;
        dstCopy.origin = {static_cast<uint32_t>(region.ImageOffsetX),
...
void WGPUCommandBufferEncoder::PipelineBarrier(PipelineStage,
                                               PipelineStage,
...
void WGPUCommandBufferEncoder::TransitionImage(const Ref<Texture>&, ImageTransition, const ImageSubresourceRange&) {}
...
void* WGPUCommandBufferEncoder::GetNativeHandle()
{
    return m_encoder;
}

void WGPUCommandBufferEncoder::ResolveTexture(const Ref<Texture>&,
                                              const Ref<Texture>&,
                                              const ImageSubresourceLayers&,
                                              const ImageSubresourceLayers&)
{}

void WGPUCommandBufferEncoder::DrawIndirect(const Ref<Buffer>& argBuffer,
                                            uint64_t offset,
                                            uint32_t drawCount,
                                            uint32_t stride)
{
...
void WGPUCommandBufferEncoder::DrawIndexedIndirect(const Ref<Buffer>& argBuffer,
                                                   uint64_t offset,
                                                   uint32_t drawCount,
                                                   uint32_t stride)
{
...
void WGPUCommandBufferEncoder::BindRayTracingDescriptorSets(const Ref<RayTracingPipeline>&,
                                                            uint32_t,
