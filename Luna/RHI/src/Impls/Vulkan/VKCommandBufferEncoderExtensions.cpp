...
namespace Cacao
{
    void VKCommandBufferEncoder::DrawIndirect(const Ref<Buffer>& argBuffer, uint64_t offset,
    ...
    void VKCommandBufferEncoder::DrawIndexedIndirect(const Ref<Buffer>& argBuffer, uint64_t offset,
                                                     uint32_t drawCount, uint32_t stride)
    {
        auto* vkBuf = static_cast<VKBuffer*>(argBuffer.get());
    ...
    void VKCommandBufferEncoder::DrawIndirectCount(const Ref<Buffer>& argBuffer, uint64_t offset,
                                                    ...
                                                    uint32_t maxDrawCount, uint32_t stride)
    {
        ...
        m_commandBuffer.drawIndirectCount(vkArg->GetHandle(), offset,
                                           vkCount->GetHandle(), countOffset,
                                           maxDrawCount, stride);
    }

    void VKCommandBufferEncoder::DrawIndexedIndirectCount(const Ref<Buffer>& argBuffer, uint64_t offset,
                                                           const Ref<Buffer>& countBuffer, uint64_t countOffset,
                                                           uint32_t maxDrawCount, uint32_t stride)
    {
        auto* vkArg = static_cast<VKBuffer*>(argBuffer.get());
        auto* vkCount = static_cast<VKBuffer*>(countBuffer.get());
        m_commandBuffer.drawIndexedIndirectCount(vkArg->GetHandle(), offset,
                                                  vkCount->GetHandle(), countOffset,
                                                  maxDrawCount, stride);
    }
    ...
    void VKCommandBufferEncoder::WriteTimestamp(const Ref<QueryPool>& pool, uint32_t queryIndex)
    {
        ...
        m_commandBuffer.writeTimestamp(vk::PipelineStageFlagBits::eBottomOfPipe,
                                       vkPool->GetNativeHandle(), queryIndex);
    }
    ...
    void VKCommandBufferEncoder::TraceRays(const Ref<ShaderBindingTable>& sbt,
                                            uint32_t w, uint32_t h, uint32_t d)
    {
        ...
        if (fn)
            fn(static_cast<VkCommandBuffer>(m_commandBuffer), &raygenRegion, &missRegion, &hitRegion, &callableRegion, w, h, d);
    }
    ...
    void VKCommandBufferEncoder::BindRayTracingDescriptorSets(
        ...
        std::span<const Ref<DescriptorSet>> descriptorSets)
    {
        ...
        m_commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eRayTracingKHR,
            vkLayout->GetHandle(), firstSet, sets, {});
    }
    ...
    void VKCommandBufferEncoder::CopyTexture2D(
        const Ref<Texture>& src, const Ref<Texture>& dst)
    {
        ...
        vkCmdBlitImage(
            ...
            static_cast<VkImage>(vkSrc->GetHandle()), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            static_cast<VkImage>(vkDst->GetHandle()), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
