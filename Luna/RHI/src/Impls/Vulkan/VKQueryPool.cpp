...
namespace Cacao
{
    static vk::QueryType ToVkQueryType(QueryType type)
    {
        ...
        case QueryType::PipelineStatistics: return vk::QueryType::ePipelineStatistics;
        default:                            return vk::QueryType::eTimestamp;
        }
    }

    VKQueryPool::VKQueryPool(Ref<VKDevice> device, const QueryPoolCreateInfo& info)
        : m_device(std::move(device)), m_type(info.Type), m_count(info.Count)
    {
    ...
    bool VKQueryPool::GetResults(uint32_t firstQuery, uint32_t queryCount,
                                 std::vector<uint64_t>& outResults, bool wait)
    {
        ...
        auto result = m_device->GetHandle().getQueryPoolResults(
            m_pool, firstQuery, queryCount,
            queryCount * sizeof(uint64_t), outResults.data(),
