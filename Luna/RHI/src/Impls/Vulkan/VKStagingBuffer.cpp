...
namespace Cacao
{
    Ref<StagingBufferPool> StagingBufferPool::Create(const Ref<Device>& device,
                                                     uint64_t blockSize,
    ...
    VKStagingBufferPool::VKStagingBufferPool(const Ref<Device>& device, uint64_t blockSize,
                                              uint32_t maxFramesInFlight)
    ...
    VKStagingBlock& VKStagingBufferPool::GetOrCreateBlock(uint64_t requiredSize)
    {
        ...
        auto buffer = std::dynamic_pointer_cast<VKBuffer>(
            m_device->CreateBuffer(bufferInfo));
