...
namespace Cacao
{
    VKRayTracingPipeline::VKRayTracingPipeline(
        const Ref<Device>& device, const RayTracingPipelineCreateInfo& info)
        : m_layout(info.Layout), m_device(device)
    {
        ...
        auto vkCreateRTPipeline = reinterpret_cast<PFN_vkCreateRayTracingPipelinesKHR>(
            dev.getProcAddr("vkCreateRayTracingPipelinesKHR"));
        if (vkCreateRTPipeline)
        {
            VkResult result = vkCreateRTPipeline(
                static_cast<VkDevice>(dev), VK_NULL_HANDLE, VK_NULL_HANDLE,
                1, &rtCI, nullptr, &m_pipeline);
