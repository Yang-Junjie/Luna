...
Ref<VKQueue> VKQueue::Create(const Ref<Device>& device, const vk::Queue& vkQueue, QueueType type, uint32_t index)
{
...
void VKQueue::Submit(const Ref<CommandBufferEncoder>& cmd, const Ref<Synchronization>& sync, uint32_t frameIndex)
{
...
void VKQueue::Submit(std::span<const Ref<CommandBufferEncoder>> cmds,
...
void VKQueue::Submit(const Ref<CommandBufferEncoder>& cmd)
{
    ...
    vk::SubmitInfo submitInfo = vk::SubmitInfo().setCommandBufferCount(1).setPCommandBuffers(&cmdHandle);
    {
        std::lock_guard<std::mutex> lock(m_submitMutex);
