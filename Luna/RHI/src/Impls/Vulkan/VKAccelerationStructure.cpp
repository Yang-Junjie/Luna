...
namespace Cacao
{
    VKAccelerationStructure::VKAccelerationStructure(
        const Ref<Device>& device, const AccelerationStructureCreateInfo& info)
        : m_type(info.Type), m_device(device)
    {
        ...
        buildInfo.flags = info.PreferFastTrace
        ...
        VkBufferCreateInfo bufCI = {}; bufCI.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        ...
        auto vkCreateAS = reinterpret_cast<PFN_vkCreateAccelerationStructureKHR>(
            dev.getProcAddr("vkCreateAccelerationStructureKHR"));
        if (vkCreateAS)
        ...
        if (info.Type == AccelerationStructureType::TopLevel && !info.Instances.empty()) {
            ...
            VkBufferCreateInfo instBufCI = {}; instBufCI.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
            ...
            for (size_t i = 0; i < info.Instances.size(); i++) {
                ...
                dst[i].instanceCustomIndex = src.InstanceID & 0xFFFFFF;
                dst[i].mask = static_cast<uint8_t>(src.Mask);
                dst[i].instanceShaderBindingTableRecordOffset = src.ShaderBindingTableOffset & 0xFFFFFF;
    ...
    VKAccelerationStructure::~VKAccelerationStructure()
    {
        ...
        if (m_accelStructure) {
            ...
            if (vkDestroyAS) vkDestroyAS(static_cast<VkDevice>(dev), m_accelStructure, nullptr);
        }
        if (m_resultBuffer) vmaDestroyBuffer(allocator, m_resultBuffer, m_resultAllocation);
        if (m_scratchBuffer) vmaDestroyBuffer(allocator, m_scratchBuffer, m_scratchAllocation);
        if (m_instanceBuffer) vmaDestroyBuffer(allocator, m_instanceBuffer, m_instanceAllocation);
    }

    uint64_t VKAccelerationStructure::GetDeviceAddress() const
    {
        auto vkDevice = std::dynamic_pointer_cast<VKDevice>(m_device);
        if (!vkDevice || !m_accelStructure) return 0;
    ...
    VkAccelerationStructureBuildGeometryInfoKHR VKAccelerationStructure::GetBuildInfo() const
    {
        ...
        info.type = (m_type == AccelerationStructureType::BottomLevel)
            ? VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR
            : VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR;
        ...
        auto vkDevice = std::dynamic_pointer_cast<VKDevice>(m_device);
        if (vkDevice)
            info.scratchData.deviceAddress = vkGetBufferDeviceAddress(
                static_cast<VkDevice>(vkDevice->GetHandle()), &scratchAddrInfo);
