...
void GLCommandBufferEncoder::End()
{
    m_recording = false;
}
    ...
    m_commands.push_back([this, info]() {
            ...
            for (uint32_t i = 0; i < info.ColorAttachments.size(); ++i) {
                ...
                if (glTex) {
                    glFramebufferTexture2D(
                        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, glTex->GetTarget(), glTex->GetHandle(), 0);
                }
            }
        } else {
        ...
        if (info.DepthAttachment) {
            if (hasValidTexture) {
                ...
                if (glDepth) {
                    glFramebufferTexture2D(
                        GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, glDepth->GetTarget(), glDepth->GetHandle(), 0);
                }
            }
...
void GLCommandBufferEncoder::BindGraphicsPipeline(const Ref<GraphicsPipeline>& pipeline)
{
    ...
    m_commands.push_back([glPipeline]() {
        if (glPipeline) {
            glPipeline->Bind();
        }
    });
...
void GLCommandBufferEncoder::BindVertexBuffer(uint32_t binding, const Ref<Buffer>& buffer, uint64_t offset)
{
    ...
    m_commands.push_back([this, binding, glBuf, offset]() {
        ...
        glVertexAttribPointer(
            binding, 4, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<const void*>(static_cast<uintptr_t>(offset)));
    });
}
...
void GLCommandBufferEncoder::BindDescriptorSets(const Ref<GraphicsPipeline>& pipeline,
                                                uint32_t firstSet,
                                                std::span<const Ref<DescriptorSet>> descriptorSets)
{
    auto glPipeline = std::dynamic_pointer_cast<GLGraphicsPipeline>(pipeline);
    m_commands.push_back(
        [glPipeline, sets = std::vector<Ref<DescriptorSet>>(descriptorSets.begin(), descriptorSets.end())]() {
...
void GLCommandBufferEncoder::PushConstants(
    const Ref<GraphicsPipeline>& pipeline, ShaderStage stageFlags, uint32_t offset, uint32_t size, const void* data)
{
    ...
    m_commands.push_back([glPipeline, offset, dataCopy]() {
        ...
        if (loc >= 0 && dataCopy.size() >= 16) {
            glUniform4fv(
                loc, static_cast<GLsizei>(dataCopy.size() / 16), reinterpret_cast<const GLfloat*>(dataCopy.data()));
        }
    });
}

void GLCommandBufferEncoder::Draw(uint32_t vertexCount,
...
void GLCommandBufferEncoder::DrawIndexed(
    ...
    m_commands.push_back([this, indexCount, instanceCount, firstIndex, vertexOffset]() {
        ...
        if (instanceCount > 1) {
            glDrawElementsInstancedBaseVertex(
                m_currentTopology, indexCount, m_currentIndexType, offset, instanceCount, vertexOffset);
        } else {
            glDrawElementsBaseVertex(m_currentTopology, indexCount, m_currentIndexType, offset, vertexOffset);
        }
    });
}
...
void GLCommandBufferEncoder::BindComputeDescriptorSets(const Ref<ComputePipeline>& pipeline,
                                                       uint32_t firstSet,
                                                       std::span<const Ref<DescriptorSet>> descriptorSets)
{
    auto glPipeline = std::dynamic_pointer_cast<GLComputePipeline>(pipeline);
...
void GLCommandBufferEncoder::ComputePushConstants(
...
void GLCommandBufferEncoder::DrawIndirect(const Ref<Buffer>& argBuffer,
                                          uint64_t offset,
                                          uint32_t drawCount,
                                          uint32_t stride)
{
...
void GLCommandBufferEncoder::DrawIndexedIndirect(const Ref<Buffer>& argBuffer,
                                                 uint64_t offset,
                                                 uint32_t drawCount,
                                                 uint32_t stride)
{
    ...
    m_commands.push_back([this, glBuf, offset, drawCount, stride]() {
        ...
        glBindBuffer(GL_DRAW_INDIRECT_BUFFER, glBuf->GetHandle());
        for (uint32_t i = 0; i < drawCount; i++) {
            glDrawElementsIndirect(m_currentTopology,
                                   m_currentIndexType,
                                   reinterpret_cast<const void*>(static_cast<uintptr_t>(offset + i * stride)));
        }
    });
...
void GLCommandBufferEncoder::CopyTexture2D(const Ref<Texture>& src, const Ref<Texture>& dst)
{
    m_commands.push_back([src, dst]() {
        ...
        glCopyImageSubData(glSrc->GetHandle(),
                           glSrc->GetTarget(),
                           0,
                           0,
                           0,
...
void GLCommandBufferEncoder::CopyBuffer(
...
void GLCommandBufferEncoder::CopyBufferToImage(const Ref<Buffer>& srcBuffer,
                                               ...
                                               std::span<const BufferImageCopy> regions)
{
    ...
    m_commands.push_back([glBuf, glTex, regions = std::vector<BufferImageCopy>(regions.begin(), regions.end())]() {
        ...
        for (auto& region : regions) {
            ...
            if (srcData) {
                glTexSubImage2D(glTex->GetTarget(),
...
void GLCommandBufferEncoder::PipelineBarrier(PipelineStage,
                                             PipelineStage,
...
void GLCommandBufferEncoder::TransitionImage(const Ref<Texture>& texture,
...
void* GLCommandBufferEncoder::GetNativeHandle()
{
    return nullptr;
}

void GLCommandBufferEncoder::ResolveTexture(const Ref<Texture>& srcTexture,
...
void GLCommandBufferEncoder::InsertDebugLabel(const std::string& name, float, float, float, float)
{
    m_commands.push_back([name]() {
        glDebugMessageInsert(
            GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_MARKER, 0, GL_DEBUG_SEVERITY_NOTIFICATION, -1, name.c_str());
    });
}
...
void GLCommandBufferEncoder::BeginQuery(const Ref<QueryPool>& pool, uint32_t queryIndex)
{
    m_commands.push_back([pool, queryIndex]() {
        ...
        if (pool->GetType() == QueryType::Occlusion) {
