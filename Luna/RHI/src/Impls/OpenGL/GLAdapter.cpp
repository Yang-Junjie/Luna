...
Ref<GLAdapter> GLAdapter::Create(const std::string& renderer, const std::string& vendor, int glMajor, int glMinor)
{
...
AdapterType GLAdapter::GetAdapterType() const
{
...
bool GLAdapter::IsFeatureSupported(DeviceFeature feature) const
{
    switch (feature) {
        ...
        case DeviceFeature::MultiDrawIndirect:
            return m_glMajor >= 4 && m_glMinor >= 3;
        case DeviceFeature::DrawIndirectCount:
            return m_glMajor >= 4 && m_glMinor >= 6;
        default:
            return false;
    }
}
...
DeviceLimits GLAdapter::QueryLimits() const
{
    ...
    limits.maxTextureSize2D = val;
    glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, &val);
    limits.maxTextureSize3D = val;
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE, &val);
    limits.maxTextureSizeCube = val;
    glGetIntegerv(GL_MAX_ARRAY_TEXTURE_LAYERS, &val);
    limits.maxTextureArrayLayers = val;
    glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS, &val);
    limits.maxColorAttachments = val;
    glGetIntegerv(GL_MAX_VIEWPORTS, &val);
    limits.maxViewports = val;
...
uint32_t GLAdapter::FindQueueFamilyIndex(QueueType) const
{
    return 0;
}
} // namespace luna::RHI
