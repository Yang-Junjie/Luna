...
namespace Cacao
{
    GLMSAARenderTarget GLMSAA::Create(uint32_t width, uint32_t height, uint32_t samples,
                                       GLenum colorFormat, bool withDepth)
    {
    ...
    void GLMSAA::Resolve(const GLMSAARenderTarget& msaaTarget, GLuint resolveFBO,
    ...
    {
        if (target.colorRenderbuffer) glDeleteRenderbuffers(1, &target.colorRenderbuffer);
        if (target.depthRenderbuffer) glDeleteRenderbuffers(1, &target.depthRenderbuffer);
