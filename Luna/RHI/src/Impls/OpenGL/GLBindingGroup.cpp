...
namespace Cacao
{
    ...
    void GLBindingGroup::AddImage(uint32_t binding, GLuint texture, uint32_t level, bool layered,
    ...
    void GLBindingGroup::Bind(GLuint program) const
    {
        ...
        for (const auto& entry : m_entries)
        {
            ...
            case GLBindingEntry::Type::Sampler:
                ...
                if (dbg) fprintf(stderr, "  Samp samp=%u err=%u\n", entry.resource, glGetError());
                break;
            case GLBindingEntry::Type::Image:
                glBindImageTexture(entry.binding, entry.resource, entry.imageLevel,
                                   entry.imageLayered ? GL_TRUE : GL_FALSE,
                                   entry.imageLayer, entry.imageAccess, entry.imageFormat);
                break;
            }
        }
    }
    ...
    {
        if (auto glTex = std::dynamic_pointer_cast<GLTexture>(
                info.TextureView ? info.TextureView->GetTexture() : nullptr))
        {
