...
bool GLPipelineCache::LoadProgram(GLuint program, const std::string& key)
{
    ...
    glProgramBinary(program, it->second.format, it->second.data.data(), static_cast<GLsizei>(it->second.data.size()));
...
{
    if (m_cacheDir.empty() || !std::filesystem::exists(m_cacheDir)) {
        return;
    }
