...
namespace Cacao
{
    static GLenum QueryTypeToGL(QueryType type)
    {
        switch (type)
        {
        case QueryType::Occlusion:          return GL_SAMPLES_PASSED;
        case QueryType::Timestamp:          return GL_TIMESTAMP;
        case QueryType::PipelineStatistics: return GL_SAMPLES_PASSED;
        default:                            return GL_SAMPLES_PASSED;
        }
    }

    GLQueryPool::GLQueryPool(const QueryPoolCreateInfo& info)
        : m_type(info.Type), m_count(info.Count)
    {
    ...
    bool GLQueryPool::GetResults(uint32_t firstQuery, uint32_t queryCount,
                                  std::vector<uint64_t>& outResults, bool wait)
    {
        ...
        for (uint32_t i = 0; i < queryCount; i++)
        {
            ...
            if (idx >= m_count) { outResults[i] = 0; continue; }
