...
bool PersistentTexture2D::ensure(const RHI::Ref<RHI::Device>& device, const PersistentTexture2DDesc& desc)
{
...
bool HistoryTexture2D::ensure(const RHI::Ref<RHI::Device>& device, const PersistentTexture2DDesc& desc)
{
    if (!isValidDesc(desc)) {
        reset();
        return false;
