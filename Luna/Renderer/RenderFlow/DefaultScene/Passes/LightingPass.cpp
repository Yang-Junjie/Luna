...
void LightingPass::setup(RenderPassContext& context)
{
    ...
    context.graph().AddRasterPass(
        name(),
        [gbuffer, shadow_map, pick_texture, scene_context](RenderGraphRasterPassBuilder& pass_builder) {
        ...
        [this, scene_context, gbuffer, shadow_map, pick_texture](RenderGraphRasterPassContext& pass_context) {
            execute(pass_context, scene_context, gbuffer, shadow_map, pick_texture);
        });
}
                           ...
                           RenderGraphTextureHandle pick_texture_handle)
{
    ...
    pipelines.updateLightingResources(
