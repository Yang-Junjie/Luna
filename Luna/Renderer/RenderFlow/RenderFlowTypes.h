...
struct SceneShaderPaths {
    ...
    std::filesystem::path post_process_path;
    ...
    [[nodiscard]] bool isComplete() const
    {
        return !geometry_vertex_path.empty() && !geometry_fragment_path.empty() && !shadow_vertex_path.empty() &&
               ...
               !transparent_composite_path.empty() && !post_process_path.empty() && !environment_ibl_path.empty();
    }
};

struct SceneRenderContext {
    RHI::Ref<RHI::Device> device;
    RHI::Ref<RHI::ShaderCompiler> compiler;
    RHI::BackendType backend_type{RHI::BackendType::Auto};
    RHI::RHICapabilities capabilities{RHI::makeCapabilitiesForBackend(RHI::BackendType::Auto)};
