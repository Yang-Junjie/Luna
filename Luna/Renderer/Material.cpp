...
Material::Material(std::string name, val::ImageData albedo_image_data, glm::vec4 albedo_color)
...
std::shared_ptr<Material>
    Material::create(std::string name, val::ImageData albedo_image_data, glm::vec4 albedo_color)
{
    return std::make_shared<Material>(std::move(name), std::move(albedo_image_data), albedo_color);
}

std::shared_ptr<Material> Material::createFromModelMaterial(const val::ModelData::Material& material_data)
{
