...
struct ImageData {
    ...
    luna::RHI::Format ImageFormat{luna::RHI::Format::UNDEFINED};
    uint32_t Width{0};
    uint32_t Height{0};
    ...
    bool isValid() const
    {
        return !ByteData.empty() && Width > 0 && Height > 0 && ImageFormat != luna::RHI::Format::UNDEFINED;
    }
};

struct CubemapData {
    std::array<std::vector<uint8_t>, 6> Faces;
    luna::RHI::Format FaceFormat{luna::RHI::Format::UNDEFINED};
