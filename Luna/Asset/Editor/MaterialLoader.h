...
inline std::shared_ptr<Material> loadFromYaml(const std::filesystem::path& path, std::string asset_name)
{
    try {
        ...
        textures.Metallic =
            loadTextureRelative(path.parent_path(), textures_node, "Metallic", asset_name + "_Metallic");
        textures.Roughness =
...
inline std::shared_ptr<Material> loadFromMtl(const std::filesystem::path& path, std::string asset_name)
{
    ...
    if (!selected_material->metallic_texname.empty() &&
        selected_material->metallic_texname == selected_material->roughness_texname) {
